...
public class Player {
    ...
    int pressed = RIGHT, block = LEFT;

    public void movementH(KeyCode key) {
        if (key == KeyCode.D && block != RIGHT && Math.abs(delay - posMem[0] + posMem[1]) > 50) {
            pressed = RIGHT;
            block = LEFT;
            delay = posMem[0] - posMem[1];
        } else {
            if (key == KeyCode.A && block != LEFT && Math.abs(delay - posMem[0] + posMem[1]) > 50) {
                pressed = LEFT;
                block = RIGHT;
                delay = posMem[0] - posMem[1];
            } else {
                if (key == KeyCode.W && block != UP && Math.abs(delay - posMem[0] + posMem[1]) > 50) {
                    pressed = UP;
                    block = DOWN;
                    delay = posMem[0] - posMem[1];
                } else {
                    if (key == KeyCode.S && block != DOWN && Math.abs(delay - posMem[0] + posMem[1]) > 50) {
                        pressed = DOWN;
                        block = UP;
                        delay = posMem[0] - posMem[1];
                    }
